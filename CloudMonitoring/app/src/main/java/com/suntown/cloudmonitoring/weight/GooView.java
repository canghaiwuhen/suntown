...
import com.suntown.cloudmonitoring.utils.DrawUtils;
import com.suntown.cloudmonitoring.utils.ScreenUtils;
...
public class GooView extends View {
    ...
    public GooView(Context context) {
        ...
        stickCircleRadius = ScreenUtils.dpToPx( context,10.0f);
        dragCircleRadius = ScreenUtils.dpToPx(context,10.0f);
        stickCircleMinRadius = ScreenUtils.dpToPx(context,3.0f);
        farest =ScreenUtils.dpToPx( context,80.0f);
        resetDistance = ScreenUtils.dpToPx(getContext(),40.0f);
    ...
    private ShapeDrawable drawGooView() {
        Path path = new Path();
        float distance = (float) DrawUtils.getDistanceBetween2Points(mDragCenter, mStickCenter);/*根据当前两圆圆心的距离计算出固定圆的半径*/
        ...
        PointF[] dragPoints = DrawUtils.getIntersectionPoints(mDragCenter, dragCircleRadius, dragLineK);
        PointF[] stickPoints = DrawUtils.getIntersectionPoints(mStickCenter, stickCircleTempRadius, dragLineK);
        //*以两圆连线的0.618处作为 贝塞尔曲线 的控制点。（选一个中间点附近的控制点）*/
        PointF pointByPercent = DrawUtils.getPointByPercent(mDragCenter, mStickCenter, 0.618f);
...
//		path.addCircle((float)stickPoints[1].x, (float)stickPoints[1].y, 5, Direction.CW);
    ...
    private float getCurrentRadius(float distance) {
        ...
        float evaluateValue = (float) ScreenUtils.evaluateValue(fraction, stickCircleRadius, stickCircleMinRadius);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (actionMasked) {
            ...
            case MotionEvent.ACTION_MOVE:{
                ...
                if (ScreenUtils.getDistanceBetween2Points(p0, p1) > farest) {
                    isOutOfRange = true;
                    updateDragCenter(event.getRawX(), event.getRawY());
                    return false;
                }
    ...
    private void handleActionUp() {
        if(isOutOfRange){
            ...
            if(ScreenUtils.getDistanceBetween2Points(mDragCenter, mInitCenter) < resetDistance){
                if(mListener != null)
            ...
            mAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    PointF pointByPercent = ScreenUtils.getPointByPercent(startPoint, endPoint, fraction);
            ...
            if(ScreenUtils.getDistanceBetween2Points(startPoint,endPoint) < 10)	{
