		...
		public void handleMessage(Message msg) {
				...
				try {
					...
					if (jsonArray.length() > 0) {
						...
						ConfirmDialog dialog = new ConfirmDialog(
								GoodsDetailActivity.this, "确定要从" + gName
										+ "附近开始导航吗?",
			...
			case MSG_GETGOODDETAIL_COMPLETE:
				...
				try {
					...
					// TODO Auto-generated catch block
					Toast.makeText(getApplicationContext(), "ERROR:商品解析错误",
							Toast.LENGTH_SHORT).show();
					e.printStackTrace();
	...
	private void initGoods(Goods goods) {
		...
		if (goods.getPriceType() != 0 && goods.getOriPriceInNumc() > 0
			...
			if (discount < 10) {
				...
				tv_discount.setText(String.format("%.1f", discount) + "折");
		...
		public void onClick(View v) {
			...
			if (goods != null) {
				...
				if (isInFavorite) {
					...
					Toast.makeText(getApplicationContext(), "已移除收藏",
							Toast.LENGTH_SHORT).show();
					((ImageView) v)
		...
		public void onComplete(boolean isOk, String msg) {
			...
			if (isOk) {
				...
				try {
					...
					if (sendState == 0) {
						// 取得订单数据，开始解析
						Toast.makeText(getApplicationContext(), "加入预购清单成功",
								Toast.LENGTH_SHORT).show();
					} else if (sendState == 1) {
						Toast.makeText(getApplicationContext(),
								"登录状态过期，请重新登录!", Toast.LENGTH_SHORT).show();
					} else {
						Toast.makeText(getApplicationContext(), "加入预购清单失败",
								Toast.LENGTH_SHORT).show();
					}
			...
			if (goods != null) {
				...
				try {
					...
					if (scdb.insertGoods(goods.getBarCode(), goods.getName(),
						...
						Toast.makeText(getApplicationContext(), "加入购物车失败",
								Toast.LENGTH_SHORT).show();
					}
					scdb.Close();
				} catch (Exception e) {
					...
					Toast.makeText(getApplicationContext(),
							"ERROR:加入购物车错误" + e.getMessage(), Toast.LENGTH_LONG)
							.show();
				}
