...
import com.suntown.scannerproject.bean.InfoBean;
...
//        Log.i(TAG, "personList-" + personList.toString());
        ...
        lvOldData.setOnItemClickListener((adapterView, view, i, l) -> {
            ...
            Intent intent1 = new Intent(HistoryDetialActivity.this, GoodsDetialActivity.class);
            intent1.putExtra(Constant.PERSON, person);
...
//    }
    ...
    @OnClick({R.id.iv_back, R.id.tv_search, R.id.tv_clear, R.id.tv_close_goods})
            ...
            case R.id.tv_clear:
                try {
                    for (Person person : oldList) {
                        ...
                        String barcode = person.barcode;
                        Log.i(TAG,"ip-"+ip+"sid-"+sid+"userId-"+userid);
                        if (null==barcode||"".equals(barcode)){
                            db.delete(person);
                        }
                        //            ip-0.0.9.115sid-0.0.9.115userId-null
