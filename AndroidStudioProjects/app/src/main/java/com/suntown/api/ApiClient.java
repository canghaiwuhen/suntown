...
import com.suntown.activity.MyApplication;
import com.suntown.netUtils.NetworkUtils;
...
public class ApiClient {
    private static final String BASE_URL = ApiConstant.formatBASE_HOST(Constant.BASE_HOST);
    ...
    static {
        ...
        File httpCacheDir = new File(MyApplication.getAppContext().getCacheDir(),"cache");

        Cache cache = new Cache(httpCacheDir, 100 * 1024 * 1024);

        Interceptor CACHE_CONTROL_INTERCEPTOR = chain -> {
            Request request = chain.request();
            if (!NetworkUtils.isAvailable(MyApplication.getAppContext())) {
